...
public class LearningRecordDelayTaskHandler {
    ...
    // CPU密集型: 与核心数相同
    // IO密集型: 核心数的两倍, 因为IO密集型任务, CPU会有空闲时间.
    ...
    /**
     * 项目启动后, 当前类实例化,属性输入之后, 方法就会运行.
     */
    @PostConstruct
    public void init(){
        log.info("LearningRecordDelayTaskHandler init方法执行了. ");
        CompletableFuture.runAsync(this::handleDelayTask); // 开启新线程执行 handlerDelayTask 方法
    }
    @PreDestroy // 当前类实例 销毁之前 该方法就会执行.
    public void destroy(){
            ...
            try {
                ...
                poolExecutor.submit(new Runnable() {
                    ...
                    public void run() {
                        ...
                        log.debug("获取到要处理的播放记录任务: {}, 缓存中的数据: {}", data, record);
                        if (record == null) {
                            return;
                        }
                        // 3.比较数据
...
//                            continue;
    ...
    public void writeRecordCache(LearningRecord record) {
        ...
        try {
            ...
            // 2.写入Redis learning:record:{lesson_id}
